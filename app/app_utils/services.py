...
def get_session_service():
    ...
    if uri := os.environ.get("SESSION_SERVICE_URI"):
        if uri.startswith(("postgresql", "sqlite")):
            from google.adk.sessions.database_session_service import DatabaseSessionService
            from app.store.engine import get_async_engine

            return DatabaseSessionService(db_engine=get_async_engine())
...
def get_memory_service():
    ...
    if uri := os.environ.get("MEMORY_SERVICE_URI"):
        ...
        service = reg.create_memory_service(uri)
